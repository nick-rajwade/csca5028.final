...
namespace components
{
    public class Store
    {
        ...
        private List<SaleItem> _inventory = null;
        ...
        public async Task StartHealthCheck()
        {
            ...
            Task sendMsg = Task.Factory.StartNew(SendHealthCheckMessage);
            await sendMsg;
        ...
        public async void StartSales()
        {
            while(true)
            {
                ...
                {
                    CreditCardProcessor processor = new CreditCardProcessor("http://localhost:9001", new System.Net.Http.HttpClient());
        ...
        public Sale GenerateSale()
        {
            ...
            if (paymentTypeIndex < 50)
            {
                paymentType = Sale.PaymentType.CreditCard;
            }
            else if (paymentTypeIndex < 75)
            {
